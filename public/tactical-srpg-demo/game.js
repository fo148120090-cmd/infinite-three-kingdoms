...
<div class="layout"><section class="panel battle"><div class="battle-head"><div><b>서쪽 관문 탈환</b><div class="hint">승리조건: 적 전멸 · 패배조건: 아군 전멸</div></div><div class="turn"><span class="turn-dot"></span>${state.turn==="hero"?"아군 턴":"적군 턴"}</div></div>
<div class="board">${Array.from({length:H},(_,y)=>Array.from({length:W},(_,x)=>{const u=unitAt(x,y),k=x+","+y;let c="cell "+terrainAt(x,y);if(moves.has(k))c+=" move";if(ats.has(k))c+=" attack";if(s&&s.x===x&&s.y===y)c+=" selected";return `<button class="${c}" onclick="cellClick(${x},${y})"><span class="coord">${x+1},${y+1}</span>${u?`<div class="unit ${u.team}"><img src="${art[u.id]}" alt=""><b>${u.name}</b><div class="hp"><i style="width:${u.hp/u.maxHp*100}%"></i></div></div>`:""}</button>`})).join("")}</div></section>
<aside class="side"><section class="panel card"><h2>출전 부대</h2><div class="unit-list">${units.filter(u=>u.team==="hero").map(u=>`<button class="unit-row ${s?.id===u.id?"active":""}" onclick="selectUnit('${u.id}')"><span class="dot"><img src="${art[u.id]}" alt=""></span><span class="unit-info"><strong>${u.name} · ${u.job}</strong><span>HP ${u.hp}/${u.maxHp} · 이동 ${u.move} · 사거리 ${u.range}${state.acted.has(u.id)?" · 행동완료":""}</span></span></button>`).join("")}</div></section>
