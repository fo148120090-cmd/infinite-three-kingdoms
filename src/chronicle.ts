...
export function systemStatus(hero:Hero):string{
  ...
  if(hero.statusNote?.startsWith("세계의 구멍 봉인 완료"))return "세계의 구멍 봉인 완료 · 전설의 원정 기록";
...
export function systemEvaluation(hero:Hero):string{
  const b=chronicleBonuses(hero);
    if(hero.statusNote?.startsWith("세계의 구멍 봉인 완료"))return "평가: 세계의 구멍을 봉인한 전설적 용사. 연대기 가산 효과가 전투력에 반영된다.";
