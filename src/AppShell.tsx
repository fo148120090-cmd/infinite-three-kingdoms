...
function BattleEngineBridge(){
 ...
 useEffect(()=>{
  const detect=()=>setActive(Boolean(document.querySelector('.battle-layout')));
  const observer=new MutationObserver(detect);observer.observe(document.body,{childList:true,subtree:true});
  const timer=window.setInterval(detect,400);detect();
