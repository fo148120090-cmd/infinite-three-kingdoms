...
const ranges:Record<string,number>={유비:2,관우:1,장비:1,조운:1,제갈량:3,조조:2,하후돈:1,손권:2,여포:4,초선:2};
...
function BattleGuide(){
 useEffect(()=>{
  ...
  const paint=()=>{
   const grids=Array.from(document.querySelectorAll('.battle-grid,.grid')) as HTMLElement[];
   const grid=grids.find(g=>g.children.length===42);
   if(!grid)return;
   ...
   cells.forEach((cell,i)=>{
    ...
    cell.style.transition='box-shadow .12s, background .12s';
   ...
   if(prev!==undefined&&prev!==selectedIndex){remaining[selected]=Math.max(0,(remaining[selected]??moves[selected])-terrainCost(selectedIndex));}
   previous[selected]=selectedIndex;
   if(remaining[selected]===undefined)remaining[selected]=moves[selected];
   const sx=selectedIndex%7,sy=Math.floor(selectedIndex/7);
   cells.forEach((cell,i)=>{
    const text=cell.textContent||''; if(!text)return;
    const x=i%7,y=Math.floor(i/7),d=Math.abs(x-sx)+Math.abs(y-sy),occupied=/🟦|🟥/.test(text),water=terrainAt(i)==='water';
    if(text.includes(selected))cell.style.boxShadow='inset 0 0 0 3px #91a0ff';
    else if(/🟥/.test(text)&&d<=ranges[selected])cell.style.boxShadow='inset 0 0 0 3px #bd5d5d';
    else if(!occupied&&!water&&d>0&&d<=remaining[selected])cell.style.boxShadow='inset 0 0 0 2px #3f8a91';
   });
  };
  const onClick=(e:MouseEvent)=>{const el=e.target as HTMLElement;const button=el.closest('button');if(button?.textContent?.includes('턴 종료'))resetMovement();if(button?.closest('.grid,.battle-grid')){window.setTimeout(paint,30);window.setTimeout(paint,180)}};
  document.addEventListener('click',onClick,true);
  const observer=new MutationObserver(paint);observer.observe(document.body,{childList:true,subtree:true});
  const timer=window.setInterval(paint,250);paint();
  return()=>{document.removeEventListener('click',onClick,true);observer.disconnect();window.clearInterval(timer)};
 },[]);
 return null;
}
function Settings(){
 const[open,setOpen]=useState(false);const[save,setSave]=useState(readSave);const refresh=()=>setSave(readSave());
