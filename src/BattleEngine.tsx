...
import { canEndPlayerTurn, createBattleState, endPlayerTurn, getBattleTargetable, getSelectedReachableCells, moveBattleUnit, selectBattleTarget, selectBattleUnit, attackBattleTarget, useBattleSkill, waitBattleUnit, type BattleState } from './game/systems/battleState';
...
export default function BattleEngine(){
 ...
 const selected=engine.units.find(u=>u.id===engine.selectedId);
 const reachable=new Set(getSelectedReachableCells(engine,terrain).map(c=>`${c.x},${c.y}`));
 const targets=new Set(getBattleTargetable(engine).map(u=>`${u.x},${u.y}`));
 const apply=(next:BattleState)=>setEngine(next);
 const clickCell=(x:number,y:number)=>{
  const unit=engine.units.find(u=>u.currentHp>0&&u.x===x&&u.y===y);
  if(unit?.team==='player'){apply(selectBattleUnit(engine,unit.id));return}
  if(unit?.team==='enemy'){apply(selectBattleTarget(engine,unit.id));return}
 ...
 const finish=()=>{if(victory){const nextFloor=Math.min(100,floor+1);const fresh={...save,floor:nextFloor,gold:(save.gold||0)+rule.reward.gold,gems:(save.gems||0)+rule.reward.gems,materials:(save.materials||0)+rule.reward.materials};localStorage.setItem(KEY,JSON.stringify(fresh));setSave(fresh)}start()};
 return <section className="be-shell">
  ...
  <div className="be-layout"><div>
   <div className="be-grid">{Array.from({length:BOARD_WIDTH*BOARD_HEIGHT},(_,i)=>{const x=i%BOARD_WIDTH,y=Math.floor(i/BOARD_WIDTH),t=terrain[i],u=engine.units.find(v=>v.currentHp>0&&v.x===x&&v.y===y),key=`${x},${y}`;return <button key={key} className={`be-cell ${terrainClass(t)} ${u?.team||''} ${u?.id===engine.selectedId?'selected':''} ${u?.id===engine.targetId?'target':''} ${reachable.has(key)?'reachable':''} ${targets.has(key)?'targetable':''}`} onClick={()=>clickCell(x,y)}><span className="be-terrain-label">{terrainLabel[t]}</span>{u&&<><b>{u.name.slice(0,2)}</b><small>{Math.max(0,u.currentHp)}/{u.maxHp}</small></>}</button>})}</div>
  ...
  </div><aside className="be-side"><h3>전투 로그</h3>{engine.log.slice(-8).reverse().map((x,i)=><p key={`${x}-${i}`}>{x}</p>)}<div className="be-roster"><h3>편성</h3>{engine.units.filter(u=>u.team==='player').map(u=><button key={u.id} className={u.id===engine.selectedId?'active':''} onClick={()=>apply(selectBattleUnit(engine,u.id))}><b>{u.name}</b><span>{u.acted?'행동 완료':`이동 ${u.movePoints}`}</span></button>)}</div></aside></div>
 </section>;
}
