...
export default function BattleEngine(){
 ...
 return <section className="be-shell"><header className="be-header"><div><small>INFINITE THREE KINGDOMS · MANUAL SRPG</small><h2>천탑 {floor}층 {rule.kind==='boss'?'· BOSS':''}</h2><p>10×10 전장 · 랜덤 지형 · 이동 → 일반공격/고유 스킬/대기 · 몬스터 AI 턴</p></div><div className={engine.turn==='player'?'be-turn':'be-turn enemy'}>{engine.turn==='player'?'PLAYER TURN':'MONSTER TURN'}</div></header>
  <div className="be-layout"><div><div className="be-grid" aria-label="10×10 랜덤 지형 전장">{presentation.cells.map(cell=>{const key=`${cell.x},${cell.y}`,u=cell.unit;const showMove=reachable.has(key)&&!u,showAttack=targets.has(key)&&u?.team==='enemy';return <button key={key} aria-label={`${cell.x+1},${cell.y+1} ${terrainLabel[cell.terrain]}${showMove?' 이동 가능':''}${showAttack?' 공격 가능':''}${showSkill?' 스킬 대상':''}`} className={`be-cell ${terrainClass(cell.terrain)} ${u?.team||''} ${cell.selected?'selected':''} ${u?.id===engine.targetId?'target':''} ${showMove?'reachable':''} ${showAttackRange?'attack-range':''} ${showSkillRange?'skill-range':''} ${showAttack?'targetable':''} ${showSkill?'skill-targetable':''}`} onClick={()=>clickCell(cell.x,cell.y)}><span className="be-terrain-label">{terrainLabel[cell.terrain]}</span>{showMove&&<span className="be-cell-hint move">이동</span>}{showAttackRange&&!showMove&&!u&&<span className="be-cell-hint range attack-range-label">공격권</span>}{showSkillRange&&!showMove&&!u&&<span className="be-cell-hint range skill-range-label">스킬권</span>}{showAttack&&<span className="be-cell-hint attack">공격</span>}{showSkill&&<span className="be-cell-hint skill">스킬</span>}{u&&<><b className="be-unit-image"><img src={getGeneralImagePaths(u).sd} alt="" onError={(e)=>{e.currentTarget.style.display='none';}}/><span>{u.name.slice(0,2)}</span></b><small>{Math.max(0,u.currentHp)}/{u.maxHp}</small>{u.status!=='none'&&<span className={`be-unit-status ${u.status}`} title={`${statusLabel[u.status]} ${u.statusTurns}턴`}>{statusBadge[u.status]}<em>{u.statusTurns}</em></span>}</>}</button>})}</div>
