...
import { getEnemyMovementCells } from './game/systems/battleView';
...
export default function BattleEngine(){
 ...
 const skillReady=Boolean(selected&&skillAvailability?.ready&&(!skillAvailability.needsTarget||(selectedTarget&&skillAvailability.targetIds.includes(selectedTarget.id))));
 ...
 return <section className="be-shell"><header className="be-header"><div><small>INFINITE THREE KINGDOMS · MANUAL SRPG</small><h2>천탑 {floor}층 {rule.kind==='boss'?'· BOSS':''}</h2><p>10×10 전장 · 랜덤 지형 · 이동 → 일반공격/고유 스킬/대기 · 몬스터 AI 턴</p></div><div className={engine.turn==='player'?'be-turn':'be-turn enemy'}>{engine.turn==='player'?'PLAYER TURN':'MONSTER TURN'}</div></header>
  <div className="be-layout"><div><div className="be-grid" aria-label="10×10 랜덤 지형 전장">{presentation.cells.map(cell=>{const key=`${cell.x},${cell.y}`,u=cell.unit;const showMove=reachable.has(key)&&!u,showAttack=targets.has(key)&&u?.team==='enemy',showSkill=skillTargets.has(key),showAttackRange=cell.attackRange,showSkillRange=cell.skillRange,showEnemyMove=enemyMovement.has(key)&&!showMove&&!u;return <button key={key} aria-label={`${cell.x+1},${cell.y+1} ${terrainLabel[cell.terrain]}${showMove?' 이동 가능':''}${showAttack?' 공격 가능':''}${showSkill?' 스킬 대상':''}`} className={`be-cell ${terrainClass(cell.terrain)} ${showEnemyMove?'enemy-move-range':''} ${u?.team||''} ${cell.selected?'selected':''} ${u?.id===engine.targetId?'target':''} ${showMove?'reachable':''} ${showAttackRange?'attack-range':''} ${showSkillRange?'skill-range':''} ${showAttack?'targetable':''} ${showSkill?'skill-targetable':''}`} onClick={()=>clickCell(cell.x,cell.y)}><span className="be-terrain-label">{terrainLabel[cell.terrain]}</span>{showMove&&<span className="be-cell-hint move">이동</span>}{showEnemyMove&&<span className="be-cell-hint enemy-move">적 이동</span>}{showAttackRange&&!showMove&&!u&&<span className="be-cell-hint range attack-range-label">공격권</span>}{showSkillRange&&!showMove&&!u&&<span className="be-cell-hint range skill-range-label">스킬권</span>}{showAttack&&<span className="be-cell-hint attack">공격</span>}{showSkill&&<span className="be-cell-hint skill">스킬</span>}{u&&<><b className="be-unit-image"><img src={getGeneralImagePaths(u).sd} alt="" onError={(e)=>{e.currentTarget.style.display='none';}}/><span>{u.name.slice(0,2)}</span></b><small>{Math.max(0,u.currentHp)}/{u.maxHp}</small>{u.status!=='none'&&<span className={`be-unit-status ${u.status}`} title={`${statusLabel[u.status]} ${u.statusTurns}턴`}>{statusBadge[u.status]}<em>{u.statusTurns}</em></span>}</>}</button>})}</div>
   <div className="be-selected-info">{selected?<><div className="be-selected-top"><div><small>선택 장수</small><h3>{selected.name}</h3><p>{selected.title} · {selected.role}</p></div><span className={`be-status-pill ${selected.status}`}>{statusLabel[selected.status]}{selected.statusTurns>0?` · ${selected.statusTurns}턴`:''}</span></div><div className="be-stat-strip"><span>HP <b>{Math.max(0,selected.currentHp)}/{selected.maxHp}</b></span><span>공격 <b>{selected.atk}</b></span><span>방어 <b>{selected.defense}</b></span><span>사거리 <b>{selected.range}</b></span><span>이동 <b>{selected.movePoints}</b></span></div><div className="be-skill-line"><b>✦ {selected.skill}</b><span>위력 {selected.skillPower||'지원형'}</span>{skillDescription&&<span className="be-skill-tooltip-wrap"><button className="be-skill-tooltip-trigger" type="button" aria-label="고유 스킬 상세 보기">?</button><span className="be-skill-tooltip" role="tooltip"><strong>{skillDescription.name}</strong><span>{skillDescription.summary}</span><span>{skillDescription.detail}</span><span>{skillDescription.targeting}</span><em>{skillDescription.powerText}</em></span></span>}</div></>:<span>장수를 선택하면 전투 스탯과 고유 스킬이 표시됩니다.</span>}</div>
   <div className="be-actions"><strong>{selected?`${selected.name} · ${selected.acted?'행동 완료':'행동 가능'}`:'장수를 선택하세요'}</strong><button onClick={()=>act(s=>attackBattleTarget(s,terrain))} disabled={!engine.targetId||!targets.has(`${engine.units.find(u=>u.id===engine.targetId)?.x},${engine.units.find(u=>u.id===engine.targetId)?.y}`)}>⚔ 일반공격</button><button onClick={()=>act(s=>useBattleSkill(s,terrain))} disabled={!skillReady} title={skillAvailability?.message||'장수를 선택하세요.'}>✦ 고유 스킬</button><button onClick={()=>act(waitBattleUnit)} disabled={!selected||selected.acted}>대기</button><button onClick={()=>act(s=>endPlayerTurn(s,terrain,floor))} disabled={!canEndPlayerTurn(engine)}>몬스터 턴 진행</button></div>
   {(victory||defeat)&&<div className="be-result"><h2>{victory?(finalFloor?'천탑 완주!':'전투 승리!'):'전투 패배...'}</h2>{victory&&<p>{finalFloor?'100층을 정복했습니다! 최종 보상을 획득했습니다.':`보상: 금화 +${rule.reward.gold} · 보옥 +${rule.reward.gems} · 재료 +${rule.reward.materials}`}</p>}<button onClick={finish}>{victory?(finalFloor?'완주 확인':'다음 층'):'다시 도전'}</button></div>}</div><aside className="be-side"><h3>전투 로그</h3><p>지형 시드: {seed}</p>{engine.log.slice(-8).reverse().map((x,i)=><p key={`${x}-${i}`}>{x}</p>)}<div className="be-roster"><h3>편성</h3>{presentation.playerUnits.map(u=><button key={u.id} className={`${u.id===engine.selectedId?'active ':''}${u.currentHp<=0?'defeated':''}`} disabled={u.currentHp<=0} onClick={()=>apply(selectBattleUnit(engine,u.id))}><b>{u.name}</b><span>{u.currentHp<=0?'전투불능':u.acted?'행동 완료':`HP ${Math.max(0,u.currentHp)} · 방어 ${u.defense}`}</span></button>)}</div></aside></div></section>;
}
