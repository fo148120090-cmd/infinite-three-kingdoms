...
export default function GeneralStatusModal(){
  ...
  const combatPower=getGeneralCombatPower(effectiveStats);
  const nextEquipLevel=Math.min(20,equipLevel+1);
  const nextEquipStats=equipLevel>=20?effectiveStats:getGeneralEffectiveStats(selected,{level,star,equipment:{...equipment,level:nextEquipLevel}});
  const nextEquipPower=getGeneralCombatPower(nextEquipStats);
