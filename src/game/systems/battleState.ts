...
function normalizeDefeatedUnits(units: Unit[]): Unit[] {
  return units.map((unit) => unit.currentHp <= 0
    ? { ...unit, currentHp: 0, status: 'none', statusTurns: 0, acted: true, movePoints: 0 }
    : unit);
}

function finalize(state: BattleState, message: string): BattleState {
  const units = normalizeDefeatedUnits(state.units);
  const outcome = isBattleOver(units);
  if (outcome === 'player') return { ...state, units, phase: 'victory', log: [...state.log, message, '전투 승리!'] };
  if (outcome === 'enemy') return { ...state, units, phase: 'defeat', log: [...state.log, message, '전투 패배...'] };
  return { ...state, units, log: [...state.log, message] };
}

function advanceStatuses(unit: Unit): Unit {
  if (unit.currentHp <= 0) return { ...unit, currentHp: 0, status: 'none', statusTurns: 0, acted: true, movePoints: 0 };
  ...
  const currentHp = Math.max(0, unit.currentHp - statusDamage);
  if (currentHp <= 0) return { ...unit, currentHp: 0, status: 'none', statusTurns: 0, acted: true, movePoints: 0 };
  return { ...unit, currentHp, statusTurns: nextTurns, status: nextTurns === 0 ? 'none' : unit.status };
}
