...
export function createMonsterEnemy(monster: Monster, index: number, floor: number): Unit {
  ...
  const placeholder: General = {
    id: `${monster.id}-${index}`, name: monster.name, title: monster.tribe, faction: 'Warlords', role: monster.role,
    hp, atk, defense, range: monster.range, move: monster.move, skill: monster.skill, skillPower: Math.floor(monster.skillPower * scale),
    ultimate: '', ultimatePower: 0, equipment: '', grade: monster.boss ? 6 : 1, tsName: '', tsTitle: '',
  };
  return {
    ...placeholder, team: 'enemy', x: BOARD_WIDTH - 1 - (index % 2), y: Math.min(BOARD_HEIGHT - 1, 1 + Math.floor(index / 2)),
    maxHp: hp, currentHp: hp, acted: false, rage: 0, buff: 0, movePoints: monster.move, status: 'none', statusTurns: 0,
  };
}
...
export function createMonsterEnemies(floor: number): Unit[] {
  ...
  const enemies = floor % 10 === 0
    ...
    : (() => {
        ...
        return ids.map(id => encounter.find(monster => monster.id === id))
