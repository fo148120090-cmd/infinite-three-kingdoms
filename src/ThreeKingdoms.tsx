  ...
  return <section className="page strategy-page" style={{height:"100%",overflow:"auto",padding:"4px 2px"}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1.4fr .6fr",gap:10,marginBottom:10}}>
      <div className="panel" style={{padding:12}}>
        ...
        {tab==="map"&&<><div style={{height:72,borderRadius:9,overflow:"hidden",marginBottom:8,border:"1px solid #2a3b55",backgroundImage:"linear-gradient(90deg,rgba(8,12,18,.82),rgba(8,12,18,.22)),url('/assets/strategy/strategy-map.svg')",backgroundSize:"cover",backgroundPosition:"center"}}><div style={{padding:9}}><span className="eyebrow">WORLD FRONT</span><b style={{display:"block",fontSize:14}}>삼국 전선 지도</b><small style={{color:"#c5cfdd"}}>도시를 선택해 원정과 국정 행동을 이어갑니다.</small></div></div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:7}}>{cities.map(c=>{const owner=save.cityOwners[c.id];const f=owner==="neutral"?undefined:factionOf(owner);return <button key={c.id} onClick={()=>setSelected(c.id)} style={{textAlign:"left",padding:10,border:"1px solid "+(selected===c.id?"#b89955":"#26364f"),borderRadius:10,background:selected===c.id?"#1c1b16":"#101927",color:"#d9e1ef",cursor:"pointer"}}><b>{c.name}</b><small style={{display:"block",color:"#8997ad",marginTop:4}}>{c.region} · {f?.name||"중립"} · 수입 {c.income}G</small><span style={{display:"block",marginTop:5,fontSize:10}}>방어 {c.defense} · 주둔 {c.garrison}</span></button>})}</div></>}
