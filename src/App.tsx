  ...
  return <div className="status-modal-backdrop" onClick={onClose}>
    <section className="status-modal" role="dialog" aria-modal="true" onClick={e=>e.stopPropagation()}>
      ...
      <div className="status-modal-grid">
        ...
        <div className="status-modal-card" style={{gridColumn:"1 / -1"}}><div className="modal-card-title"><b>초월 각성</b><span>{awakeningSummary(hero)}</span></div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8}}>{[50,70,100].map(level=>{const unlocked=(hero.awakenings||[]).some(x=>x.level===level);const data=(hero.awakenings||[]).find(x=>x.level===level);return <div key={level} style={{padding:10,borderRadius:10,border:"1px solid "+(unlocked?"#8f7440":"#26364f"),background:unlocked?"#17140d":"#0c1421"}}><b>{level===50?"🌱 개화":level===70?"⚡ 극점":"♾️ 무한"}</b><small style={{display:"block",marginTop:5,color:unlocked?"#d7c18c":"#6f7c90"}}>{unlocked?data?.name:"Lv."+level+" 해금"}</small><small style={{display:"block",marginTop:4,color:"#9aa9bf"}}>{unlocked?data?.detail:"레벨 도달 시 영구 성장 보너스 획득"}</small></div>})}</div></div>
