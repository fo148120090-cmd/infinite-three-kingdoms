...
const combatStats=(hero:Hero)=>{
  ...
  const hp=Math.round(hero.hp*starMult*(1+((m.hpPct||0)+(artifactMods.hpPct||0)+(bonus.hpPct||0)+passive.hpPct+promotion.hpPct+costumeBonus.hpPct)/100));
  return {hp,maxHp:hp,attack:Math.round(hero.attack*starMult)+(m.attack||0)+(artifactMods.attack||0)+(bonus.attack||0)+passive.attack+promotion.attack+costumeBonus.attack,defense:Math.round(hero.defense*starMult)+(m.defense||0)+(artifactMods.defense||0)+(bonus.defense||0)+passive.defense+promotion.defense+costumeBonus.defense,speed:hero.speed*(1+((m.speedPct||0)+(artifactMods.speedPct||0)+(bonus.speedPct||0)+passive.speedPct+promotion.speedPct+costumeBonus.speedPct)/100),range:hero.range+(m.range||0)+(artifactMods.range||0)+passive.range+promotion.range+costumeBonus.range};
};
