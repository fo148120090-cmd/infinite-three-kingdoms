      ...
      <div className="character-equipment-panel">
        ...
        <div className="character-equipment-layout">
          ...
          <div className="character-equipment-inventory">
            <div className="character-inventory-head"><div><b>{hero.name}에게 장착 가능한 장비</b><span>선택 슬롯 · {selectedEquipSlot+1}</span></div><span>{save.items.length}/60</span></div>
            {save.items.length===0?<div className="quick-empty">창고가 비어 있습니다. 전투 전리품과 보물방에서 장비를 획득하세요.</div>:<div className="character-inventory-grid">{save.items.map((item,idx)=>{const d=equipmentPreview(hero,item,selectedEquipSlot);const deltas=[d.attack&&"공격 "+(d.attack>0?"+":"")+d.attack,d.defense&&"방어 "+(d.defense>0?"+":"")+d.defense,d.hp&&"HP "+(d.hp>0?"+":"")+d.hp,d.speed&&"속도 "+(d.speed>0?"+":"")+Math.round(d.speed*100)/100,d.range&&"사거리 "+(d.range>0?"+":"")+d.range].filter(Boolean) as string[];return <article className={"character-inventory-item "+(item.unique?"unique":"")} key={item.id+"-"+idx}><div className="character-inventory-item-top"><span>{item.slot}</span>{item.unique&&<b>UNIQUE</b>}</div><strong>{item.name}</strong><small>{item.rarity} · Lv.{item.level}</small><div className="equip-delta">{deltas.slice(0,3).map((x,i)=><em key={i}>{x}</em>)}</div><button className="primary-btn compact" onClick={()=>equip(item,selectedEquipSlot)}>슬롯 {selectedEquipSlot+1}에 장착</button></article>})}</div>}
          </div>
        </div>
      </div>
