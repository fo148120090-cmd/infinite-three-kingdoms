...
function actionForecast(hero:Hero,partyHeroes:Hero[],partyMemory?:PartyMemory):{action:string;score:number;detail:string}[]{
  ...
  const mods=combinedAiMods(items);
...
export default function App(){
  ...
  return <main className="game-shell">
    ...
    {screen==="party"&&<section className="page"><div className="section-head"><div><span className="eyebrow">CHARACTERS</span><h2>원정대 구성</h2><p className="muted">전투 전에만 편성과 장비를 변경할 수 있습니다.</p></div><span className="counter">{save.party.length}/4</span></div>
      ...
      <div className="subpanel"><div><b>현재 편성</b><span>{party.map(h=>jobIcon[h.job]+" "+h.name).join(" · ")}</span></div><div className="social-summary"><span>관계는 전투를 함께할수록 강화되고, 동료를 잃으면 기억이 남습니다.</span></div><button className="primary-btn compact" onClick={()=>setScreen("dungeon")}><Swords size={16}/> 던전으로</button></div><div className="character-status-overview"><div className="character-status-identity"><span className="eyebrow">CHARACTER STATUS</span><div className="character-status-name"><div className="hero-avatar large" style={{background:hero.color}}>{jobIcon[hero.job]}</div><div><h3>{hero.name}</h3><p>{jobKo[hero.job]} · {promotionLabel(hero)} · Lv.{hero.level}</p><span>{buildProfile(hero).name} · {chronicleLabel(hero)}</span></div></div><div className="character-status-notes"><span><b>기분</b>{systemMood(hero)}</span><span><b>상태</b>{systemStatus(hero)}</span><span><b>평가</b>{systemEvaluation(hero)}</span></div></div><div className="character-stat-block"><div className="status-block-head"><b>기본 스탯</b><span>보정 전 기본값</span></div><div className="character-stat-grid"><span><small>HP</small><strong>{Math.round(hero.hp)}</strong></span><span><small>공격</small><strong>{Math.round(hero.attack)}</strong></span><span><small>방어</small><strong>{Math.round(hero.defense)}</strong></span><span><small>속도</small><strong>{Math.round(hero.speed*100)/100}</strong></span><span><small>사거리</small><strong>{Math.round(hero.range*100)/100}</strong></span><span><small>경험</small><strong>{hero.experience}/100</strong></span></div><div className="status-applied"><span>현재 전투 적용</span><b>HP {Math.round(heroCombatStats.maxHp)} · 공격 {Math.round(heroCombatStats.attack)} · 방어 {Math.round(heroCombatStats.defense)} · 속도 {Math.round(heroCombatStats.speed*100)/100} · 사거리 {Math.round(heroCombatStats.range*100)/100}</b></div></div><div className="character-status-summary"><div className="status-block-head"><b>연대기 · 전투 기록</b><span>누적</span></div><div className="status-bonus-line">연대기 가산 · 공격 +{chronicleBonus.attack||0} · 방어 +{chronicleBonus.defense||0} · HP +{chronicleBonus.hpPct||0}% · 속도 +{chronicleBonus.speedPct||0}%</div><div className="profile-metrics compact-metrics"><span><strong>{hero.combatProfile?.battles||0}</strong>전투</span><span><strong>{hero.combatProfile?.actions||0}</strong>행동</span><span><strong>{hero.combatProfile?.damage||0}</strong>피해</span><span><strong>{hero.combatProfile?.healing||0}</strong>회복</span></div><div className="profile-actions compact-actions">{Object.entries(hero.combatProfile?.topActions||{}).sort((x,y)=>y[1]-x[1]).slice(0,3).map(([name,count])=><em key={name}>{name} · {count}회</em>)}</div></div></div><div className="party-memory-panel"><div><span className="eyebrow">PARTY MEMORY</span><b>파티 집단 기억</b><p>개별 캐릭터의 습관과 별개로, 함께 싸운 경험이 보호·회복·생존 판단에 남습니다.</p></div><div className="party-memory-metrics"><span><strong>{save.partyMemory?.battles||0}</strong>협동 전투</span><span><strong>{save.partyMemory?.protection||0}</strong>보호 행동</span><span><strong>{save.partyMemory?.recovery||0}</strong>회복 행동</span><span><strong>{save.partyMemory?.losses||0}</strong>패배 경험</span></div><small>{partyMemorySummary(save.partyMemory)}</small></div>
      ...
      <div className="quick-equipment-panel"><div className="quick-equipment-head"><div><span className="eyebrow">QUICK EQUIP</span><b>{hero.name} 장비 관리</b><small>슬롯을 선택한 뒤 창고 장비의 <strong>장착</strong>을 누르면 바로 교체됩니다.</small></div><button className="ghost-btn" onClick={()=>setScreen("inventory")}><Package size={15}/> 전체 창고</button></div><div className="quick-slots">{equipmentSlotsOf(hero).map((item,slot)=><div key={slot} className={"quick-slot "+(selectedEquipSlot===slot?"selected":"")}><button onClick={()=>setSelectedEquipSlot(slot)} className="quick-slot-main"><small>SLOT {slot+1}</small><b>{item?.name||"장비 없음"}</b><span>{item?item.rarity+" · Lv."+item.level:"비어 있음"}</span></button>{item&&<button className="ghost-btn" onClick={()=>unequip(slot)}>해제</button>}</div>)}</div><div className="quick-warehouse"><div className="quick-warehouse-title"><b>창고 장비</b><span>현재 선택 슬롯 · {selectedEquipSlot+1}</span></div>{save.items.length===0?<span className="quick-empty">장착 가능한 창고 장비가 없습니다. 전투 전리품과 보물방에서 획득합니다.</span>:<div className="quick-item-list">{save.items.map((item,idx)=><div className={"quick-item "+(item.unique?"unique":"")} key={item.id+"-"+idx}><div><b>{item.name}</b><span>{item.rarity} · Lv.{item.level} · {item.slot}</span><small>{item.stats.slice(0,2).join(" · ")}</small><div className="equip-delta">{(()=>{const d=equipmentPreview(hero,item,selectedEquipSlot);const parts=[d.attack&&"공격 "+(d.attack>0?"+":"")+d.attack,d.defense&&"방어 "+(d.defense>0?"+":"")+d.defense,d.hp&&"HP "+(d.hp>0?"+":"")+d.hp,d.speed&&"속도 "+(d.speed>0?"+":"")+Math.round(d.speed*100)/100,d.range&&"사거리 "+(d.range>0?"+":"")+Math.round(d.range*100)/100].filter(Boolean);return <>{parts.map((x,i)=><em key={i}>{x}</em>)}{d.aiDelta.map(x=><em key={x.key}> {tendencyKo[x.key]} {x.value>0?"+":""}{Math.round(x.value*10)/10}</em>)}</>})()}</div></div><button className="primary-btn compact" onClick={()=>equip(item,selectedEquipSlot)}>장착</button></div>)}</div>}</div></div>
  ...
  return <article className={"hero-card "+(active?"hero-selected":"")} onClick={onClick}>
    ...
    <div className="hero-card-main">
      ...
      <div className="social-meta">
        ...
        <span>{bond?"전술 링크 · "+(bond.relation.bond>=25?"활성":"형성 중"):"전술 링크 · 미형성"}</span>
