...
export default function App(){
  ...
  return <main className="game-shell">
    ...
    {screen==="inventory"&&(()=>{const filtered=save.items.filter(i=>warehouseTab==="all"||i.slot===warehouseTab);const rarityRank:Record<string,number>={신화:5,전설:4,영웅:3,희귀:2,일반:1};const sorted=filtered.slice().sort((a,b)=>warehouseSort==="level"?b.level-a.level:warehouseSort==="rarity"?(rarityRank[b.rarity]||0)-(rarityRank[a.rarity]||0):0);const selectedItem=sorted.find(i=>i.id===selectedWarehouseItem)||sorted[0];return <section className="page"><div className="section-head"><div><span className="eyebrow">GUILD WAREHOUSE · INVENTORY</span><h2>용사단 창고</h2><p className="muted">아이템을 수집하고 선택한 슬롯에 바로 장착합니다. 모든 직업이 모든 장비를 사용할 수 있습니다.</p></div><span className="counter">{save.items.length} / 60</span></div>
      ...
      <div className="warehouse-layout"><div className="warehouse-main"><div className="equipment-slots">{equipmentSlotsOf(hero).map((item,slot)=><div key={slot} className={"equipment-slot "+(selectedEquipSlot===slot?"selected":"")}><button onClick={()=>setSelectedEquipSlot(slot)} className="slot-main"><small>SLOT {slot+1}</small><b>{item?.name||"장비 없음"}</b><span>{item?item.rarity+" · Lv."+item.level:"아이템을 선택해 장착"}</span></button>{item&&<button className="ghost-btn slot-action" onClick={()=>unequip(slot)}>해제</button>}</div>)}</div>
        <div className="warehouse-grid">{sorted.length===0?<div className="subpanel empty-warehouse"><b>해당 카테고리에 장비가 없습니다.</b><span>전투 전리품과 보물방에서 장비를 획득하세요.</span></div>:sorted.map((i,idx)=><button key={i.id+"-"+idx} className={"warehouse-item "+(selectedItem?.id===i.id?"selected ":"")+(i.unique?"unique":"")} onClick={()=>setSelectedWarehouseItem(i.id)}><div className="warehouse-item-icon">{i.slot==="weapon"?"⚔":i.slot==="armor"?"🛡":i.slot==="ring"?"◈":"✦"}</div><div className="warehouse-item-head"><span>{i.rarity}</span>{i.unique&&<b>U</b>}</div><strong>{i.name}</strong><small>Lv.{i.level}</small><div className="warehouse-item-stats">{i.stats.slice(0,2).map(s=><span key={s}>{s}</span>)}</div></button>)}</div></div>
        <aside className="warehouse-detail">{selectedItem?<><div className="warehouse-detail-head"><span>{selectedItem.rarity}</span>{selectedItem.unique&&<b>UNIQUE</b>}</div><h3>{selectedItem.name}</h3><p>{selectedItem.description}</p><div className="warehouse-detail-meta"><span>종류 {selectedItem.slot}</span><span>레벨 {selectedItem.level}</span><span>장착 슬롯 {selectedEquipSlot+1}</span></div><div className="stat-list">{selectedItem.stats.map(s=><span key={s}>{s}</span>)}</div><div className="ai-mod"><Brain size={14}/>{Object.entries(selectedItem.aiMods).map(([k,v])=><span key={k}>{tendencyKo[k as keyof Tendencies]} {(v||0)>0?"+":""}{v}</span>)}</div><div className="warehouse-detail-actions"><button className="primary-btn" onClick={()=>equip(selectedItem,selectedEquipSlot)}>슬롯 {selectedEquipSlot+1}에 장착</button><button className="ghost-btn danger-btn" onClick={()=>{sellItem(selectedItem);setSelectedWarehouseItem(undefined);}}>판매</button></div></>:<div className="warehouse-empty-detail"><span>ITEM</span><b>장비를 선택하세요</b><small>인벤토리에서 아이템을 클릭하면 상세 정보가 표시됩니다.</small></div>}</aside>
      </div></section>})()}
