...
export default function App(){
  ...
  return <main className="game-shell">
      ...
      <div className="character-equipment-panel">
        ...
        <div className="character-equipment-layout">
          ...
          <div className="character-equipment-inventory">
            ...
            {save.items.length===0?<div className="quick-empty">인벤토리가 비어 있습니다. 전투 전리품과 보물방에서 장비를 획득하세요.</div>:<div className="character-inventory-grid">{save.items.map((item,idx)=><div className="inventory-icon-wrap" key={item.id+"-"+idx}><button className={"inventory-icon "+(item.unique?"unique":"")} onClick={()=>equip(item,selectedEquipSlot)} title={item.name+"\\n"+item.rarity+" · Lv."+item.level+"\\n"+item.stats.join(" · ")+"\\nAI · "+Object.entries(item.aiMods).map(([k,val])=>tendencyKo[k as keyof Tendencies]+" "+((val||0)>0?"+":"")+(val||0)).join(" · ")+"\\n"+item.description} aria-label={item.name+" 장착"}><span>{item.slot==="weapon"?"⚔":item.slot==="armor"?"🛡":item.slot==="ring"?"◈":"✦"}</span><small>Lv.{item.level}</small>{item.unique&&<b>U</b>}</button></div>)}</div>}
          </div>        </div>
      </div>
    ...
    {screen==="inventory"&&(()=>{const filtered=save.items.filter(i=>warehouseTab==="all"||i.slot===warehouseTab);const rarityRank:Record<string,number>={신화:5,전설:4,영웅:3,희귀:2,일반:1};const sorted=filtered.slice().sort((a,b)=>warehouseSort==="level"?b.level-a.level:warehouseSort==="rarity"?(rarityRank[b.rarity]||0)-(rarityRank[a.rarity]||0):0);return <section className="page"><div className="section-head"><div><span className="eyebrow">GUILD WAREHOUSE · INVENTORY</span><h2>용사단 인벤토리</h2><p className="muted">아이콘 중심으로 간소화했습니다. 커서를 올리면 이름·희귀도·스탯·AI 보정·설명이 표시되고, 클릭하면 현재 선택 슬롯에 장착됩니다.</p></div><span className="counter">{save.items.length} / 60</span></div>
      ...
      <div className="warehouse-icon-grid">{sorted.length===0?<div className="subpanel empty-warehouse"><b>해당 카테고리에 장비가 없습니다.</b><span>전투 전리품과 보물방에서 장비를 획득하세요.</span></div>:sorted.map((item,idx)=><div className="inventory-icon-wrap" key={item.id+"-"+idx}><button className={"inventory-icon warehouse-icon "+(item.unique?"unique":"")} onClick={()=>equip(item,selectedEquipSlot)} title={item.name+"\\n"+item.rarity+" · Lv."+item.level+"\\n"+item.stats.join(" · ")+"\\nAI · "+Object.entries(item.aiMods).map(([k,val])=>tendencyKo[k as keyof Tendencies]+" "+((val||0)>0?"+":"")+(val||0)).join(" · ")+"\\n"+item.description} aria-label={item.name+" 장착"}><span>{item.slot==="weapon"?"⚔":item.slot==="armor"?"🛡":item.slot==="ring"?"◈":"✦"}</span><small>Lv.{item.level}</small>{item.unique&&<b>U</b>}</button><button className="inventory-sell" onClick={()=>sellItem(item)} aria-label={item.name+" 판매"}>×</button></div>)}</div>
    </section>})()}
