...
import { normalizePassiveData, passiveAiBonus, passiveCombatBonus, passiveMilestones, passiveSetFor, passiveSpentPoints, upgradePassive } from "./passiveSkills";
...
export default function App(){
  ...
  const resetHeroPassive=(heroId:string)=>{
    const target=save.heroes.find(h=>h.id===heroId);
    if(!target)return;
    const spent=passiveSpentPoints(target);
    if(spent<=0){notify(target.name+" · 초기화할 패시브 포인트가 없습니다.");return;}
    const cost=250+spent*20;
    if(save.gold<cost){notify("패시브 초기화 비용 부족 · "+cost+"G 필요");return;}
    const set=passiveSetFor(target);
    const resetLevels:Record<string,number>={};
    for(const skill of set.skills) resetLevels[skill.id]=0;
    const next={...target,passiveSkills:resetLevels,skillPoints:(target.skillPoints||0)+spent,history:["패시브 초기화 · "+spent+"P 반환",...(target.history||[])].slice(0,6)};
    setSave(s=>({...s,gold:s.gold-cost,heroes:s.heroes.map(h=>h.id===heroId?next:h)}));
    notify(target.name+" · 패시브 초기화 완료 · "+spent+"P 반환 · -"+cost+"G");
  };
