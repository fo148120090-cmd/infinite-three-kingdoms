...
export default function App(){
  ...
  <div className="result-promotion-panel"><div className="report-head"><b>⚔ 전직 패시브 전투 기록</b><span>이번 전투에 적용된 전직 효과</span></div><div className="promotion-result-grid">{battle.units.filter(u=>u.team==="player").map(u=>{const h=save.heroes.find(x=>x.id===u.id);const passive=h?promotionPassive(h):undefined;return <div className="promotion-result-card" key={u.id}><div><strong>{h?.name||u.name}</strong><span>{h?.promotionPath?.[h.promotionPath.length-1]||"기본 전직"}</span></div><b>✦ {passive?.name||"전직 패시브 없음"}</b><small>{passive?.detail||"현재 적용 중인 전직 전용 패시브가 없습니다."}</small>{passive&&<div className="promotion-result-stats">{passive.attack>0&&<span>공격 +{passive.attack}</span>}{passive.defense>0&&<span>방어 +{passive.defense}</span>}{passive.hpPct>0&&<span>HP +{passive.hpPct}%</span>}{passive.speedPct>0&&<span>속도 +{passive.speedPct}%</span>}{passive.range>0&&<span>사거리 +{passive.range}</span>}{passive.healPct>0&&<span>치유 +{passive.healPct}%</span>}</div>}<em>전투 행동 {(u.battleStats?.actions||0)}회 · 치명타 {(u.battleStats?.critical||0)}회</em></div>})}</div></div>
  <div className="result-costume-panel"><div className="report-head"><b>✦ 코스튬 전투 기록</b><span>이번 전투에 적용된 코스튬 효과</span></div><div className="costume-result-grid">{battle.units.filter(u=>u.team==="player").map(u=>{const h=save.heroes.find(x=>x.id===u.id);const skinId=h?.equippedSkinId||h?.costumeId||u.skinId;const passive=costumePassive(skinId);return <div className="costume-result-card" key={u.id}><div><strong>{h?.name||u.name}</strong><span>{skinId?skinLabel(skinId):"기본 코스튬"}</span></div><b>✦ {passive.name}</b><small>{passive.detail}</small><div className="costume-result-stats">{passive.attack>0&&<span>공격 +{passive.attack}</span>}{passive.defense>0&&<span>방어 +{passive.defense}</span>}{passive.hpPct>0&&<span>HP +{passive.hpPct}%</span>}{passive.speedPct>0&&<span>속도 +{passive.speedPct}%</span>}{passive.range>0&&<span>사거리 +{passive.range}</span>}{passive.healPct>0&&<span>치유 +{passive.healPct}%</span>}{passive.critPct>0&&<span>치명 +{passive.critPct}%</span>}</div><em>{u.battleStats?.costumeFx||0}회 발동 · 치명타 {(u.battleStats?.critical||0)}회</em></div>})}</div></div>
