...
const combatStats=(hero:Hero)=>{
  ...
  const costumeBonus=ownedCostumeCombatBonus(hero.skinIds||[hero.equippedSkinId||hero.costumeId||hero.job.toLowerCase()+"-base"]);
  const hp=Math.round(hero.hp*starMult*(1+((m.hpPct||0)+(artifactMods.hpPct||0)+(bonus.hpPct||0)+passive.hpPct+costumeBonus.hpPct)/100));
  return {hp,maxHp:hp,attack:Math.round(hero.attack*starMult)+(m.attack||0)+(artifactMods.attack||0)+(bonus.attack||0)+passive.attack+costumeBonus.attack,defense:Math.round(hero.defense*starMult)+(m.defense||0)+(artifactMods.defense||0)+(bonus.defense||0)+passive.defense+costumeBonus.defense,speed:hero.speed*(1+((m.speedPct||0)+(artifactMods.speedPct||0)+(bonus.speedPct||0)+passive.speedPct+costumeBonus.speedPct)/100),range:hero.range+(m.range||0)+(artifactMods.range||0)+passive.range+costumeBonus.range};
};
...
export default function App(){
  ...
  return <main className="game-shell">
    ...
    {screen==="battle"&&<section className="page play-page battle-page"><div className="battle-header"><div><span className="eyebrow">{roomKo[battle.room]}</span><h2>{battle.room==="evilCave"?"악의 동굴 · 세계의 구멍":battle.room==="boss"?"심층 관문":battle.repeatScenarioFloor!==undefined?"시나리오 재도전":"자동 전투 진행 중"}</h2><p className="muted">전투 명령 없음 · 일시정지와 재생 속도만 조절할 수 있습니다.</p></div>
      ...
      <div className={"battle-scene battle-scene-"+battle.mode+" battle-room-"+battle.room+" battle-phase-"+battle.phase}><div className="battle-sky-glow"/><div className="battle-vignette"/><div className="battle-banner-left">⚔ FRONTIER ARMY</div><div className="battle-banner-right">ENEMY FRONT ⚠</div>{battle.mode==="raid"&&<div className={"raid-phase-hud raid-phase-"+battle.phase}><span>PHASE {battle.phase}</span><b>{battle.phase===1?"전초 · 패턴 분석":battle.phase===2?"격화 · 전장 압박":"종언 · 보스 폭주"}</b><small>보스 HP {Math.round((pct(battle.units.find(u=>u.team==="enemy"&&u.grade==="Boss")||{hp:0,maxHp:1}))*100)}%</small></div>}<div className="battle-layout" style={{gridTemplateColumns:"1fr"}}><div className="cave-panel"><div className="cave-label"><span>입구</span><span>심층</span></div><div className="cave-lane"><div className="cave-floor"/>
        {battle.units.map(u=><div key={u.id} className={"battle-unit "+u.team+" "+(u.alive?"":"dead")+" "+(active?.id===u.id?"active-unit":"")+" "+(u.fxKind?"fx-"+u.fxKind:"")+" "+(u.job?"unit-job-"+u.job.toLowerCase():"")+" "+(u.grade?"unit-grade-"+String(u.grade).toLowerCase():"")} style={{left:(u.pos*9.3)+"%"}}>
          <div className="unit-token-frame"><div className={"unit-token art-character-frame "+(u.team==="player"?"art-hero-frame":"art-enemy-frame")}>{u.team==="player"?<SkinPortrait job={u.job!} skinId={save.heroes.find(h=>h.id===u.id)?.equippedSkinId} compact />:<div className={"monster-asset monster-asset-"+String(u.grade||"Normal").toLowerCase()} aria-hidden="true"><span>{u.grade==="Boss"?"♛":u.grade==="Named"?"☠":u.grade==="Elite"?"◆":"◇"}</span></div>}{u.team==="enemy"&&<span className={"monster-level-badge grade-"+String(u.grade||"Normal").toLowerCase()}>Lv.{u.level||1}</span>}{u.team==="player"&&<span className="unit-job-badge">{jobIcon[u.job!]}</span>}</div><span className="unit-scanline"/></div><div className="unit-nameplate"><b>{u.name}</b><span>{u.team==="player"?(u.job?jobKo[u.job]:"전투원"):(u.species||"적대 개체")}</span></div>{u.team==="enemy"&&<small className="monster-meta">{u.species} · {u.grade||"Normal"}{u.promotionTier?(" · 승급 "+u.promotionTier+"단계"):""}</small>}{u.mutation&&<small className="mutation-label">{u.mutation}</small>}{u.fx&&<span className="combat-fx">{u.fx}</span>}<div className="hp-bar"><span style={{width:(100*pct(u))+"%"}}/></div><small className="unit-hp-text">{Math.max(0,Math.round(u.hp))}/{u.maxHp}</small></div>)}
    ...
    {statusHeroId&&save.heroes.find(h=>h.id===statusHeroId)&&<CharacterStatusModal heroes={save.heroes} hero={save.heroes.find(h=>h.id===statusHeroId)!} onClose={()=>setStatusHeroId(undefined)} onNavigate={id=>setStatusHeroId(id)} onTranscend={transcendHero} onUpgradePassive={upgradeHeroPassive}/>}
    <footer><span>Prototype · autonomous dungeon AI</span><button onClick={reset}><RotateCcw size={14}/> 초기화</button></footer>
  </main>;
}

function CharacterStatusModal({hero,heroes,onClose,onNavigate,onTranscend,onUpgradePassive}:{hero:Hero;heroes:Hero[];onClose:()=>void;onNavigate:(id:string)=>void;onTranscend:(heroId:string)=>void;onUpgradePassive:(heroId:string,skillId:string)=>void}){
...
function HeroCard({hero,active,onClick,onStatus}:{hero:Hero;active:boolean;onClick:()=>void;onStatus:()=>void}){
  ...
  return <article className={"hero-card "+(active?"hero-selected":"")} onClick={onClick}>
    ...
    <div className="hero-card-main">
      ...
      <small>기재·특성 · {[...(hero.traits||[]),...(hero.artifacts||[])].join(" · ")||"없음"}</small>
